...
[INDEX TABLE  每条 32B，共 N 条，按 key 字符串升序排列]
  key_offset [8B  LE uint64] → string pool 偏移
  word_offset[8B  LE uint64] → string pool 偏移
  ini_offset [8B  LE uint64] → string pool 偏移 (initials)
...
版本：v2 — pool offset 改为 uint64，支持 >4GB 字符串池。
...
MAGIC          = b'JIUYI001'
FORMAT_VERSION = 2        # v2: pool offset 改为 uint64
HEADER_SIZE    = 32
ENTRY_SIZE     = 32       # key_off(8) + word_off(8) + ini_off(8) + freq(4) + flags(4)
MAX_WORD_LEN   = 100
MAX_FREQ       = 2_147_483_647

# INDEX TABLE 打包格式： uint64 key_off, uint64 word_off, uint64 ini_off, int32 freq, uint32 flags
...
def build_bin(entries, out_path: str):
    ...
    INDEX TABLE 每条 32B：
      key_off(8) + word_off(8) + ini_off(8) + freq(4、int32) + flags(4、uint32)
    """
    ...
    index_offset = pool_offset + pool_size
    ...
    with open(out_path, 'wb') as f:
        # Header (32B)
        f.write(MAGIC)                                  # 8B
        f.write(struct.pack('<I', FORMAT_VERSION))       # 4B
        f.write(struct.pack('<I', entry_count))          # 4B
        f.write(struct.pack('<Q', index_offset))         # 8B
        f.write(struct.pack('<Q', 0))                    # 8B reserved
        ...
        for ko, wo, io, freq, lang_flag in index_rows:
            f.write(_ENTRY_STRUCT.pack(ko, wo, io, freq, lang_flag))

    size_mb = out.stat().st_size / 1024 / 1024
    print(f"[build] 完成！{entry_count:,} 条 → {out_path} ({size_mb:.1f} MB)")
    if size_mb > 150:
        print("[warn ] 文件超过 150MB，建议检查是否有大量重复词条。")
...
def verify_bin(bin_path: str):
    ...
    def read_str(offset: int) -> str:
        ...
        return data[pool_start + offset + 2: pool_start + offset + 2 + length].decode('utf-8', errors='replace')

    zh_count = en_count = 0
    for i in range(entry_count):
        base    = index_offset + i * ENTRY_SIZE
        flags,  = struct.unpack_from('<I', data, base + 28)
        if flags & 1:
            zh_count += 1
        else:
            en_count += 1
    ...
    for i in range(min(5, entry_count)):
        base = index_offset + i * ENTRY_SIZE
        ko, wo, io, freq, flags = _ENTRY_STRUCT.unpack_from(data, base)
        key      = read_str(ko)
        word     = read_str(wo)
    ...
    for i in range(entry_count):
        base = index_offset + i * ENTRY_SIZE
        ko,  = struct.unpack_from('<Q', data, base)
        key  = read_str(ko)
        if key < prev_key:
            unsorted += 1
            if unsorted <= 3:
